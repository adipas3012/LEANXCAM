...
//----------------------------------------------------------------------
//Ab hier werden die Zeitwerte eingestellt
...
void InitProcess() {
	...
	fprintf(fs,"<tr><th>Schrittnummer</th><th>Farbe</th><th>Biggest Area</th><th>AreaAvarage</th><th>B</th><th>G</th><th>R</th></tr> \n");
	fclose(fs);
...
void ChangeDetection() {
	int row, col, cpl;
	int analysehoehe = 180;
	int grenzeoben = ((240-analysehoehe)/2)*376;
	int grenzeunten = (240-(240-analysehoehe)/2)*376;


	//loop over the rows
	for(row = grenzeoben; row < grenzeunten; row += nc) {
...
void Activated(struct OSC_PICTURE *picIn, struct OSC_VIS_REGIONS *regions, s_color color)
{
	...
	if(framediff == timetodetect){
		...
		//Relevante Werte werden in Textdatei geschrieben / FARBANALYSE. Diese Datei ist aufrufbar unter 192.168.1.10/werte.txt und kann z.B. in Excel ausgewertet werden.
		FILE *we = fopen("/home/httpd/werte.txt", "a");
		fprintf(we,"%d, %d, %d, %d, %d, %d, %d \n", data.ipc.state.nStepCounter, framediff, BiggestArea, BiggestAreaAvarage, coloravarage[0], coloravarage[1], coloravarage[2]);
		fclose(we);
...
//Hier werden die Farbbereiche definiert. Dir Arrays sind folgendermassen aufgebaut: int farbe[6] = {Min. Blau, Max. Blau, Min. Grün, Max. Grün, Min. Rot, Max. Rot}
	...
	const char *a[7];
	const char *c[7];
	//Definiert die verschiedenen Cases 0-6 mit den entsprechenden Titel (a) und den Tabellenhintergrundfarben (c)
	a[0] = "unklar"; c[0] = "";
...
//Überprüfung ob Gummibärchen weiss ist
	if (data.ipc.state.nSortOutWhite == 1 && white[0] <= coloravarage[0] && white[1] >= coloravarage[0]  && white[2] <= coloravarage[1] && white[3] >= coloravarage[1]  && white[4] <= coloravarage[2] && white[5] >= coloravarage[2])
	{
		...
		if(data.ipc.state.nSortOutWhite == 1){
		color = 1;}
		//Grössenüberprüfung
...
//Überprüfung ob Gummibärchen dunkelrot (darkred) ist
	else if (data.ipc.state.nSortOutRed == 1 && darkred[0] <= coloravarage[0] && darkred[1] >= coloravarage[0]  && darkred[2] <= coloravarage[1] && darkred[3] >= coloravarage[1]  && darkred[4] <= coloravarage[2] && darkred[5] >= coloravarage[2])
	{
		...
		casenumber = 2;
		//Überprüfen der Auswahl aus dem Webinterface
		if(data.ipc.state.nSortOutRed == 1){
		color = 1;}
		//Grössenüberprüfung
...
//Überprüfung ob Gummibärchen grün (green) ist
	...
	//Überprüfung ob Gummibärchen orange ist
...
if(SecondBigAvarage > 2000){
	size = 0;
}


	//Relevante Werte werden in Textdatei geschrieben / STATISTIKFILE
...
void ControlGPIO(struct OSC_PICTURE *picIn, struct OSC_VIS_REGIONS *regions){
	...
	if (data.ipc.state.nStepCounter-timestamp[0] == waitforaction){
		//Hier kann eingestellt werden wie lange der Ausgang eingeschaltet bleibt
		gpiotimer += durationofaction;
